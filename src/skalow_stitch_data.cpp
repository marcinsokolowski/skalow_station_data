...
/*void AddExtraKeys( CBgFits& out_image )
{
     if( !out_image.GetKeyword("CTYPE1") ){
        out_image.SetKeyword("CRPIX1",1); // or 0.5-(1,1)=0.059MHz, 1->(1,1)=0Mhz-CORRECT, 0->(1,1)->0.117MHz
        out_image.SetKeyword("CTYPE1","Frequency");
        out_image.SetKeyword("CUNIT1","MHz");
        out_image.SetKeyword("CDELT1","0.11718750");
     }
     if( !out_image.GetKeyword("CTYPE2") ){
        out_image.SetKeyword("CRPIX2",1); // or 0.5-(1,1)=0.059MHz, 1->(1,1)=0Mhz-CORRECT, 0->(1,1)->0.117MHz
        out_image.SetKeyword("CTYPE2","Time");
        out_image.SetKeyword("CUNIT2","sec");
        out_image.SetKeyword("CDELT2","0.546133333");
     } 
}*/
...
//     n_out_channels = n_out_channels - 1;
  ...
  CBgFits out( n_out_channels , fits.GetYSize() );
  out.SetValue(0.00);
  ...
  out.PrepareBigHornsHeader( fits.dtime_fs + fits.dtime_fu/1000000.00, fits.inttime, start_freq, fits.delta_freq );
...
*/   
  ...
  for(int ch=0;ch<(n_fine_ch-(nc/2-1));ch++){
     for(int t=0;t<fits.GetYSize();t++){
        double val = fits.getXY(ch,t);
        out.setXY(ch,t,val);
     }
     out_channel_index++;     
  }
...
//  out_channel_index--; // step back as the next channel needs to be averaged with last from previous
  ...
  for(int file=1;file<fits_list_vec.size();file++){
     ...
     for(int t=0;t<fits.GetYSize();t++){
        double val1 = out.getXY(out_channel_index-1,t); // adding to previously filled channel, for the 2nd coarse channel it will be 119-1=118 to be averaged with 10th (nc/2) from 2nd coarse channel
        double val2 = fits.getXY(nc/2,t); // to be averaged with 10th (nc/2) from 2nd coarse channel
        
        out.setXY( out_channel_index-1,t,(val1+val2)/2.00);
     }
//     out_channel_index++;
     ...
     for(int ch=(nc/2+1);ch<last_ch;ch++){ // channel nc/2 was already added and averaged with the channel from previous Coarse Channel, so we need to start from nc/2+1 = 11th channel (counting from ZERO)
        for(int t=0;t<fits.GetYSize();t++){
           double val = fits.getXY(ch,t);
           out.setXY( out_channel_index,t,val); 
        }
